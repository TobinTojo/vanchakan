-- Fix 409 conflicts during evidence generation (unique evidence_order on reorder)

CREATE OR REPLACE FUNCTION shuffle_evidence_order(p_room_id UUID)
RETURNS VOID AS $$
BEGIN
  -- Move orders out of the way so reassignment cannot hit UNIQUE(room_id, evidence_order)
...
EXCEPTION
  WHEN unique_violation THEN
    -- Another client may have finished; advance if evidence exists
    IF (SELECT COUNT(*) FROM evidence WHERE room_id = p_room_id) > 0 THEN
      UPDATE rooms SET status = 'evidence', phase_ends_at = now() + interval '15 seconds', updated_at = now()
